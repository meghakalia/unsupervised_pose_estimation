...
def evaluate(opt):
    ...
    folder_names = glob.glob(os.path.join(opt.log_dir, '**/B_SelfSupervised-ArtifactRemoval-NoPoseLoss-LongtermLoss/Output/*'))
    ...
    for skip_frame_freq in opt.frame_skip:
        ...
        # for traj in range(1,5):
            ...
            # if filenames_1 and filenames_1[0].split('/')[2] == 'BoxPhantom2' and filenames_1[0].split('/')[5][:-4] == 'forward':
                ...
                np.savez(os.path.join(save_dir, 'pose_prediction_{}.npz'.format(27)), a = pred_poses)
                np.savez(os.path.join(save_dir,'eulerangle_{}.npz'.format(27)), a = axisangle_)
                np.savez(os.path.join(save_dir,'translation_{}.npz'.format(27)), a = translation_)
