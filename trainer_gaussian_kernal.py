...
experiment_name = "gauss_mixture_samesigmaNetwork_recon_pretrained_trainable_dataaug_{}_gauss_num_{}_batchnorm_{}_ssim_l1_{}_sigma_network_gauss_combination{}_same_gausskernel_{}_separatemeanstd_{}".format(data_aug, gauss_number, batch_norm, frac, bool_multi_gauss, same_gauss_kernel, separate_mean_std)
# wandb 
config = dict(
    height = height,
    width = width,
...
if not train_unet_only: 
    ...
    if separate_mean_std: 
        models['sigma_combined'] = networks.FCN(output_size = 15)
        ...
        for g in range(1, gauss_number+1): 
            models['sigma{}'.format(g)] = networks.FCN(output_size = 5) # 4 for each of std x, std y, mean x , mean y, two for the mixture of gaussians 
...
for  epoch in range(num_epochs):
    ...
    for batch_idx, inputs in enumerate(train_loader):
        ...
        for frame_id in [0, -1, 1]:
            ...
            losses = compute_reprojection_loss(outputs['compose'], inputs["color", frame_id, 0], frac)
        ...
        if ( step + 1) %  save_frequency == 0:
            with torch.no_grad():
                ...
                features_val        = models["decompose"](inputs["color", 0, 0])
                ...
                wandb.log({"{}".format('train_original'):wandb.Image(inputs["color", 0, 0], caption ='original image'),'custom_step':custom_step})  
